...
def create_products_impressions(queryset):
    ...
    for index, product in enumerate(queryset):
        impressions.append(dict(id=product.reference, \
                name=product.name, price=product.get_price(), brand='BTP_STORE', \
...
class ProductsView(generic.ListView):
    ...
    def get_queryset(self, **kwargs):
        ...
        try:
            ...
            raise http.Http404("Collection inexistante")
        else:
...
class PreviewProductView(LoginRequiredMixin, generic.DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not request.user.is_admin:
            return http.HttpResponseForbidden('Accès interdit à cette page')
        return content
...
class SearchView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # TODO
        collections = ['electricplomb', 'construction']
...
class SpecialOfferView(generic.DetailView):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            return redirect('shop_gender', 'grosoeuvre')
        else:
            return product
...
def alter_item_quantity(request, **kwargs):
    ...
    if cart_id:
        ...
        try:
            ...
            messages.error(request, _("Panier inexistant"), extra_tags='alert-warning')
            return redirect('checkout')
        else:
        ...
        messages.error(
            request, _("Panier inexistant"), extra_tags='alert-warning')
    return redirect('checkout')
