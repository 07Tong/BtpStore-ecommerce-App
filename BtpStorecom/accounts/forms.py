...
class UserLoginForm(auth_forms.AuthenticationForm):
    ...
    def clean(self):
        ...
        if email is None and password:
            raise forms.ValidationError(_("Veuillez saisir un email ainsi qu'un mot de passe"))    
        ...
        if email and password:
            ...
            if self.user_cache:
                ...
                raise forms.ValidationError("Vous n'êtes pas autorisé à vous connecter")
...
class BaseProfileForm(forms.ModelForm):
    class Meta:
        ...
        fields  = ['name', 'surname']
        widgets = {
            'name': widgets.TextInput(attrs={'placeholder': 'Jiomekong'}),
            'surname': widgets.TextInput(attrs={'placeholder': 'Azanzi'}),
        }
...
class AddressProfileForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'address': widgets.TextInput(attrs={'placeholder': 'Bastos'}),
            'city': widgets.TextInput(attrs={'placeholder': 'Yaoundé'}),
            'zip_code': widgets.TextInput(attrs={'placeholder': '59120'})
        }


class AnonymousUserForm(forms.Form):
    address = fields.CharField(max_length='100', required=True, widget=widgets.TextInput(attrs={'autocomplete': 'address-line-1', 'placeholder': 'Bastos'}))
    city = fields.CharField(max_length='45', required=True, widget=widgets.TextInput(attrs={'autocomplete': 'city', 'placeholder': 'Yaoundé'}))
    ...
    country = fields.CharField(max_length='45', widget=widgets.Select(attrs={'autocomplete': 'country', 'placeholder': 'Cameroun'}, choices=(('cameroun', 'Cameroun'),)))
